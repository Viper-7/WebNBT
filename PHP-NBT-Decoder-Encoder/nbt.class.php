...
extension_loaded("gmp") || extension_loaded("bcmath") or trigger_error (
	"The NBT class requires the BCMath or GMP extension for 64-bit number handling. ".
	...
	public function readType($fp, $tagType) {
		switch($tagType) {
			...
			case self::TAG_LONG: // Signed long (64 bit, big endian)
				($gmp = extension_loaded("gmp")) || extension_loaded("bcmath") or trigger_error (
		...
		switch($tagType) {
			...
			case self::TAG_LONG: // Signed long (64 bit, big endian)
				...
				if($gmp) {
					...
					$secondHalf = bcmod($value, 2147483647);
					$firstHalf = bcsub($value, $secondHalf);
					return is_int(fwrite($fp, pack("N", $firstHalf))) && is_int(fwrite($fp, pack("N", $secondHalf)));
				}
			case self::TAG_FLOAT: // Floating point value (32 bit, big endian, IEEE 754-2008)
